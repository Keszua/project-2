...
class GiftRecord {
    constructor(obj) {
        ...
        if (!obj.count || obj.count < 1 || obj.count > 999999) {
            throw new ValidationError('Liczba szt. prezentów powinna się mieścić w przedziale od 1 do 999999.');
        }
    ...
    static async getOne(id) {
        ...
        console.log("GiftRecord getone", results[0]);
        return results.length === 0 ? null : new GiftRecord(results[0]);
    }

    async countGivenGifts() {
        const [[{count}]] = await pool.execute("SELECT COUNT(*) AS `count` FROM `children` WHERE `gift_id` = :id", {
            id: this.id,
        });
        return count;
    }
